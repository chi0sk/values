...
function walkupdate()
	...
	-- Combine the inputs into the priority string
	if w then
		newPriority = "W"
	end
	if a then
		newPriority = newPriority .. "A"
	end
	if s then
		newPriority = newPriority .. "S"
	end
	if d then
		newPriority = newPriority .. "D"
	end

	-- Only update if the priority has changed
	if newPriority ~= lastpriority then
		dochange = newPriority
		dochange2 = newPriority
...
function usethatgun(owner)
	...
	if died==false then
		...
		if gunview == "Butterfly Knife" or gunview == "Huntsman Knife" or gunview == "Bayonet" or gunview == "Karambit" then
			equip:AdjustSpeed(0.1)
			equip:Play()
			equip:AdjustSpeed(0.1)
		else
			equip:Play()
...
function firebullet(fanfire)
	...
	coroutine.wrap(function()
		...
		local y = math.cos(r) * (math.random(0 , 100)/100)]]
									...
									--(UpperTorso.Velocity.magnitude/(game.ReplicatedStorage.HUInfo[gun.Name].WalkSpeed.Value*hammerunit2stud)) -- chi0sk :3
									local veloc=0
									local bop=false
		...
		pcall(function()
			...
			if script.Images:FindFirstChild(item).Value == Page or Page == "All" then
				if Inventory:FindFirstChild(table2string(CurrentInventory[i])) == nil then
					local Split = string.split(CurrentInventory[i][1], "_")
					
					local template = script.WeaponTemplate:Clone()
					template.Name = i
					template.Parent = Inventory
					
					template.NameLabel.Text = Split[1] .. " | " .. Split[2]
					template.Wep.Image = script.Images:FindFirstChild(Split[1]):FindFirstChild(Split[2]).Value
					
					if Split[2] ~= "Stock" then
						template.Quality.BackgroundColor3 = script.Colors:FindFirstChild(script.Images:FindFirstChild(Split[1]):FindFirstChild(Split[2]).Quality.Value).Value
					else
						template.Quality.BackgroundColor3 = script.Colors:FindFirstChild("Stock").Value
					end
					
					if CurrentInventory[i][2] > 1 then
						template.amount.Text = "x"..CurrentInventory[i][2]
						template.amount.Visible = true
					else
						template.amount.Visible = false
					end
...
local eeeee = script.Parent:WaitForChild("GUI")

eeeee.TeamSelection.Red.ImageLabel.MouseEnter:Connect(function()
	eeeee.TeamSelection.Red.Select.Visible = true
	script.Parent.Sounds.MenuHover:Play()
end)

eeeee.TeamSelection.Blue.ImageLabel.MouseLeave:Connect(function()
	eeeee.TeamSelection.Blue.Select.Visible = false
end)

eeeee.TeamSelection.Red.ImageLabel.MouseLeave:Connect(function()
	eeeee.TeamSelection.Red.Select.Visible = false
end)
...
_Run.RenderStepped:Connect(function()
	...
	--[[if gun and gun:FindFirstChild("Scoped") then
		...
		if Humanoid and Humanoid.Health>0 then
			...
			if Character and Character:FindFirstChild("Crouched") then
				if UpperTorso and UpperTorso.Velocity.magnitude>=1 and crouchwalkanim then
					-- Load animation based on movement direction
					local moveDir = ""
					if UserInputService:IsKeyDown(Enum.KeyCode.W) then moveDir = moveDir.."W" end
					if UserInputService:IsKeyDown(Enum.KeyCode.A) then moveDir = moveDir.."A" end
					if UserInputService:IsKeyDown(Enum.KeyCode.S) then moveDir = moveDir.."S" end
					if UserInputService:IsKeyDown(Enum.KeyCode.D) then moveDir = moveDir.."D" end

					if moveDir ~= dochange then
						if crouchwalkanim and crouchwalkanim.IsPlaying then
							crouchwalkanim:Stop(2*(crouchcooldown/4))
						end
						crouchwalkanim = Humanoid:LoadAnimation(script.Crouching[moveDir.."Walk"])
						dochange = moveDir
					end

					if crouchwalkanim and not crouchwalkanim.IsPlaying then
						crouchwalkanim:Play(math.max(.4,2*(crouchcooldown/4)))
					end
					...
					if crouchwalkanim and crouchwalkanim.IsPlaying then
						crouchwalkanim:Stop(math.max(.4,2*(crouchcooldown/4)))
					end
				end
			end
